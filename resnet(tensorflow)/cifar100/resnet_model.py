...
def cifar10_resnet_v2_generator(resnet_size, num_classes, data_format=None, more_layer=False):
  ...
  def model(inputs, is_training):
    ...
    inputs = block_layer(
        inputs=inputs, filters=16, block_fn=bottleneck_block, blocks=num_blocks,
        strides=1, is_training=is_training, name='block_layer1',
        data_format=data_format)
    inputs = block_layer(
        inputs=inputs, filters=32, block_fn=bottleneck_block, blocks=num_blocks,
        strides=2, is_training=is_training, name='block_layer2',
        data_format=data_format)
    inputs = block_layer(
        inputs=inputs, filters=64, block_fn=bottleneck_block, blocks=num_blocks,
    ...
    inputs = tf.reshape(inputs, [-1, 64 * 4])
    if not more_layer:
        inputs = tf.layers.dense(inputs=inputs, units=num_classes)
        inputs = tf.identity(inputs, 'first_dense')
    else:
        inputs = tf.layers.dense(inputs=inputs, units=300)
        inputs = tf.identity(inputs, 'first_dense')
        inputs = tf.layers.dense(inputs=inputs, units=num_classes)
