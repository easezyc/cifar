...
parser.add_argument('--wordvec', type=bool, default=True,
                    help='Whether use wordvec.')

parser.add_argument('--lr_boundaries', type=list, default=[100, 200, 300],
                    help='The boundaries for learning rate.')

parser.add_argument('--lr_values', type=list, default=[0.2, 0.05, 0.02, 0.01],
...
parser.add_argument('--more_layer', type=bool, default=False,
                    help='The number of images per batch.')
...
def cal_d_cifar():#计算d_cifar
    ...
    for i in range(100):
        d_cifar100[cifar100.iloc[i, 0]] = np.array(cifar100.iloc[i, 1:301])#(np.array(cifar100.iloc[i, 1:301]) + 1)/2
...
def cifar100_model_fn(features, labels, mode, params):
  ...
  network = resnet_model.cifar10_resnet_v2_generator(
      params['resnet_size'], _NUM_CLASSES, params['data_format'], more_layer=FLAGS.more_layer)
  ...
  if FLAGS.wordvec:
    cross_entropy = tf.reduce_mean(tf.reduce_sum(tf.squared_difference(labels, logits), 1))
  else:
...
def save_usepre_model():
    wordmodel_dir = "./tmp/cifar100_model_res%d_word_weightdecay%f/" % (FLAGS.resnet_size, _WEIGHT_DECAY)
    network = resnet_model.cifar10_resnet_v2_generator(
        FLAGS.resnet_size, _NUM_CLASSES, FLAGS.data_format, FLAGS.more_layer)
...
if __name__ == '__main__':
  ...
  if FLAGS.wordvec:
      global _NUM_CLASSES
      _NUM_CLASSES = 300
      FLAGS.more_layer = False
      FLAGS.model_dir = "./tmp/cifar100_model_res%d_word_weightdecay%f" % (FLAGS.resnet_size, _WEIGHT_DECAY)
      FLAGS.log_dir = "./log/cifar100_log_res%d_word_weightdecay%f" % (FLAGS.resnet_size, _WEIGHT_DECAY)
      main(unparsed)
      #tf.app.run(argv=[sys.argv[0]] + unparsed)

  global _NUM_CLASSES
  _NUM_CLASSES = 100
  FLAGS.model_dir = "./tmp/cifar100_model_res%d_useword_weightdecay%f" % (FLAGS.resnet_size, _WEIGHT_DECAY)
  FLAGS.log_dir = "./log/cifar100_log_res%d_useword_weightdecay%f" % (FLAGS.resnet_size, _WEIGHT_DECAY)
  FLAGS.more_layer = True
  FLAGS.lr_boundaries = [50, 100, 150, 200, 250, 300, 350]
  FLAGS.lr_values = [0.4, 0.2, 0.1, 0.05, 0.02, 0.01, 0.005, 0.002] 
  FLAGS.wordvec = False
  save_usepre_model()
  tf.app.run(argv=[sys.argv[0]] + unparsed)
